...
from gst_automation.gst.observation import GstObservationEngine


@dataclass(frozen=True, slots=True)
class GstObservationSessionJobHandler(JobHandlerV2):
    """Operator-driven GST observation session (records everything for one successful workflow)."""
    ...
    async def run_with_context(
        ...
        try:
            ...
            await GstObservationEngine(settings=settings).run(
                ctx.session,
                r,
                job_id=job_id,
